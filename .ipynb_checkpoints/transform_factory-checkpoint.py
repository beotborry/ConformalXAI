...
class AddGaussianNoise(object):
    ...
    def __call__(self, img):
        ...
        noise = torch.randn(img_tensor.size()) * (self.std)**0.05 + self.mean
...
class TransformFactory():
    def __init__(self, interpolation = InterpolationMode.BICUBIC, fill = None, num_trans = 2, noise_std = 0.01) -> None:
        self.interpolation = interpolation
        self.fill = fill
    ...
    def __call__(self, img:Image.Image) -> Tensor:
        space = ['hflip', 'rotation', 'color_jitter', 'grayscale', 'gaussian_noise']
        trans_list = random.sample(space, self.num_trans)

        trans = []

        for op_name in trans_list:
            if op_name == 'hflip': trans.append(transforms.RandomHorizontalFlip(1)),
            elif op_name == 'rotation':
                degree = np.random.choice([90., 180., 270.], 1)[0]
                trans.append(transforms.RandomRotation((degree, degree), self.interpolation))
            elif op_name == 'color_jitter':
                trans.append(self.color_jitter)
            elif op_name == 'grayscale':
                trans.append(transforms.RandomGrayscale(p=1))

        if "gaussian_noise" in trans_list:
            tmp = [
